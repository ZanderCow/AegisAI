...
async def test_keyword_flagged_message_returns_moderation_response(
    ...
    with patch("src.api.v1.endpoints.chat.validate_provider", return_value=None):
        resp = await client.post(
...
async def test_keyword_flagged_message_stores_event(
    ...
    async with TestingSessionLocal() as session:
        result = await session.execute(select(FlaggedEvent))
...
@pytest.mark.asyncio
async def test_clean_message_is_not_flagged(client, auth_headers, groq_conversation_id):
