...
_DOCUMENT_ROLES = {"user", "admin", "security"}
...
async def upload_document(
    ...
    allowed_roles: str = Form(default=""),  # comma-separated: "user,admin"
    ...
    - **allowed_roles**: Comma-separated roles, e.g. ``user,admin``.
    """
    ...
    invalid_roles = sorted(set(roles) - _DOCUMENT_ROLES)
    if invalid_roles:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=(
                "Invalid allowed_roles value(s): "
                f"{', '.join(invalid_roles)}. Valid roles are: {', '.join(sorted(_DOCUMENT_ROLES))}."
            ),
        )
    ...
    svc = DocumentService(DocumentRepository(db), rag)
    try:
...
async def update_document(
    ...
    Only admin users may update documents. Role changes take effect
    immediately — the next RAG query will use the updated Postgres
    allowed_roles without any ChromaDB re-indexing required.
    """
    _require_role(auth, _ADMIN_ROLES)
